...
const navItems: NavItem[] = [
  {
    icon: <GridIcon />,
    name: "Dashboard",
    subItems: [{ name: "Ecommerce", path: "/admin/dashboard", pro: false }],
  },
  {
    icon: <CalenderIcon />,
    name: "Calendar",
...
const AppSidebar: React.FC = () => {
  ...
  const routePrefix = location.pathname.startsWith("/superadmin")
    ? "/superadmin"
    : "/admin";
  const resolvePath = useCallback(
    (path: string) =>
      path.startsWith("/admin")
        ? path.replace("/admin", routePrefix)
        : path,
    [routePrefix]
  );
  ...
  const isActive = useCallback(
    (path: string) => location.pathname === resolvePath(path),
